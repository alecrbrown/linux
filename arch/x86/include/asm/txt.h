...
static inline void txt_parse_heap(void *heap, struct txt_heap_info *txt_heap_ptrs[TXT_SINIT_TABLE_MAX])
{
	void *tmp = heap;
	for (u8 i; i < TXT_SINIT_TABLE_MAX; i++) {
		txt_heap_ptrs[i]->size = *((u64 *) tmp);
		txt_heap_ptrs[i]->offset = tmp - heap + sizeof(txt_heap_ptrs[i]->size);
		tmp += txt_heap_ptrs[i]->size;
	}
}
