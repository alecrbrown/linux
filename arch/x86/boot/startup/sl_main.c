...
static bool __init sl_setup_data_entry_over_4g(const struct setup_data *entry)
{
	struct setup_indirect *ind;
...
static void __init sl_extend_setup_data(struct slr_policy_entry *entry, bool under4g)
{
	...
	if (!under4g) {
		 ...
		 */
		while (data && !sl_setup_data_entry_over_4g(data))
	 ...
	 */
	while (data) {
		if (under4g && sl_setup_data_entry_over_4g(data))
			break;
